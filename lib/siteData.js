...
// Urdu translations for each management member's role + bio, keyed by
// name (names themselves stay in Roman script, as is conventional).
const MANAGEMENT_UR = {
  "Muhammad Hashim Khan": {
    role: "چیئرمین ایم پی ایل",
    copy: "نظم و ضبط اور پیشہ ورانہ مہارت کے ساتھ ایم پی ایل کے وژن، حکمتِ عملی، اور ترقی کی قیادت کرتے ہیں، تاکہ اسے مانیری کے لوگوں کے لیے عمدگی، اتحاد، فیئر پلے، اور فخر کی علامت بنایا جا سکے۔",
  },
  "Shahzad Ali Shah": {
    role: "پرو چیئرمین",
    copy: "چیئرمین کی معاونت کرتے ہیں، لیگ کے آپریشنز کی نگرانی کرتے ہیں، اور ٹورنامنٹ کے کامیاب انعقاد کو یقینی بناتے ہیں۔",
  },
  "Tanveer Jamal": {
    role: "ڈائریکٹر کمیونیکیشنز",
    copy: "سرکاری ابلاغ، ٹیم کوآرڈینیشن، تعلقاتِ عامہ، اور معلومات کی ہموار ترسیل کا انتظام کرتے ہیں۔",
  },
  "Abdullah, Seyal Khan, Shamas Khan": {
    role: "آپریشنز اینڈ ایونٹس مینجمنٹ",
    copy: "شیڈولنگ، گراؤنڈ انتظامات، لاجسٹکس، اور ٹیم کوآرڈینیشن کو سنبھالتے ہیں۔",
  },
  "Mir Azam Khan": {
    role: "لیگ رولز اینڈ لیگل ایڈوائزر",
    copy: "فیئر پلے کو یقینی بناتے ہیں، قواعد کا انتظام کرتے ہیں، تنازعات حل کرتے ہیں، اور قانونی رہنمائی فراہم کرتے ہیں۔",
  },
  "M Shayan Khan, M Huzeefa, Junaid Banaras": {
    role: "میڈیا اینڈ براڈکاسٹنگ",
    copy: "لائیو کوریج، جھلکیاں، فوٹوگرافی، اور ڈیجیٹل میڈیا کا مواد سنبھالتے ہیں۔",
  },
  "Adil Khan": {
    role: "فنانس منیجر",
    copy: "بجٹ سازی، اخراجات، انعامات کی تقسیم، اور شفاف مالی انتظام کو سنبھالتے ہیں۔",
  },
};

export function getManagement(lang) {
  if (lang !== "ur") return MANAGEMENT;
  return MANAGEMENT.map((member) => {
    const ur = MANAGEMENT_UR[member.name];
    return ur ? { ...member, role: ur.role, copy: ur.copy } : member;
  });
}
