...
import { getTermsArticles, getTermsMeta, getTermsUndertaking } from "@/lib/termsData";
import { useLanguage } from "@/lib/i18n/LanguageContext";
import LanguageToggle from "@/components/site/LanguageToggle";

export default function TermsContent() {
  const { lang, t } = useLanguage();
  const termsMeta = getTermsMeta(lang);
  const termsArticles = getTermsArticles(lang);
  const termsUndertaking = getTermsUndertaking(lang);
      ...
      <div className="mx-auto w-[min(900px,calc(100%-32px))]">
        ...
        <div className="rounded-lg border border-ink/10 bg-white p-6 shadow-panel sm:p-9">
          <div className="mb-8 flex flex-col items-center gap-3 border-b-2 border-gold/60 pb-7 text-center">
            ...
            <p className="font-bold text-muted">{termsMeta.issuedBy}</p>
            <p className="max-w-2xl text-[0.95rem] font-semibold text-ink/80">{termsMeta.intro}</p>
          </div>
              ...
              <section key={article.heading} className="border-b border-ink/10 pb-6 last:border-b-0 last:pb-0">
                  ...
                  <div className="mt-3 grid gap-2.5 sm:grid-cols-3">
                    {article.levels.map((lvl) => (
                      <div key={lvl.level} className="rounded-lg border border-ink/10 bg-[#fbfbf8] p-3.5">
                        ...
                        <p className="text-[0.82rem] font-extrabold text-green-dark">
                          {t("terms.penalty")}: {lvl.penalty}
                        </p>
                      </div>
                    ))}
                  </div>
                ...
                {article.penalty ? (
                  <div className="mt-2.5 rounded-lg bg-[#ffe3df] px-3.5 py-2.5">
                    <p className="mb-1 text-[0.82rem] font-black text-brand-red">{t("terms.penalty")}</p>
          ...
          <div className="mt-8 rounded-lg border-2 border-gold/60 bg-[#fbfbf8] p-6">
            <h2 className="mb-3 text-[1.05rem] font-black text-navy-dark">{termsUndertaking.intro}</h2>
            <ul className="grid list-disc gap-2.5 pl-5 text-[0.9rem] font-semibold text-ink/85">
              {termsUndertaking.items.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
            <p className="mt-4 font-black text-green-dark">{termsUndertaking.closing}</p>
          </div>
        </div>
      </div>
    </main>
