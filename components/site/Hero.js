    ...
    >
      <div className="mx-auto grid w-[min(1180px,calc(100%-32px))] items-center gap-8 pb-10 pt-[74px] sm:min-h-[610px] lg:grid-cols-[minmax(0,1fr)_360px] lg:pb-[54px] lg:pt-16">
        ...
        <aside
          ...
          aria-label={t("hero.summaryAria")}
          style={{ animationDelay: "480ms" }}
        >
          <strong className="block text-[2rem] leading-none text-navy-dark">{t("hero.establishedDate")}</strong>
          <span className="block font-bold text-muted">{t("hero.establishedCaption")}</span>
        </aside>
      </div>
    </header>
  );
